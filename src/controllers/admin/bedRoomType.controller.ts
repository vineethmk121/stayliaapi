...
export default {
    async addBedRoomType(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            const existingBedRoomType = await bedRoomTypeModel.findOne({ title: req.body.title }).lean();
            ...
            const bedRoomType: bedRoomTypeData = new bedRoomTypeModel(req.body);
    ...
    async viewBedRoomType(req: Request, res: Response, next: NextFunction) {
        try {
            const bedRoomType: bedRoomTypeData = await bedRoomTypeModel.findById(req.params.id).lean();
            if (!bedRoomType) {
    ...
    async getAllBedRoomTypes(req: Request, res: Response, next: NextFunction) {
        try {
            var bedRoomType: bedRoomTypeData = await bedRoomTypeModel.find({ delBit: false }).lean();
            if (!bedRoomType) {
    ...
    async updateBedRoomType(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            var bedRoomType: bedRoomTypeData = await bedRoomTypeModel.findByIdAndUpdate(req.params.id, req.body, { new: true }).select('-password').lean();
            if (!bedRoomType) {
    ...
    async deleteBedRoomType(req: Request, res: Response, next: NextFunction) {
        try {
            var bedRoomType: bedRoomTypeData = await bedRoomTypeModel.findByIdAndUpdate(req.params.id, { delBit: true }, { new: true }).select('-password').lean();
            if (!bedRoomType) {
                let result = makeApiResponce('BedRoomType Not Exists', 1, StatusCodes.BAD_REQUEST, bedRoomType);
                return res.status(StatusCodes.BAD_REQUEST).json(result);
            }
